...
export default function CheckInFeedback({ profile }) {
  ...
  const [location, setLocation] = useState(""); // 新增：定觀地點狀態
  ...
  // 模擬獲取當日地點（實務上可從 profile 或當日活動 table 抓取）
  useEffect(() => {
    if (profile?.volunteer_group) {
      setLocation(`${profile.volunteer_group}例行定觀點`);
    }
  }, [profile]);
